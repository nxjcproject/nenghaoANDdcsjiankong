...
function loadPowerData() {
    if (m_GetDataFlag == true) {
        var m_PowerBarInfo = $('#Text_PowerBarInfo').val();   //OrganizationId
        $.ajax({
            type: "POST",
            url: "MultiMonitorShell.aspx/GetPowerBarData",
            data: "{myPowerBarInfo:'" + m_PowerBarInfo + "',myOrganizationId:'" + 'zc_nxjc_qtx_efc'  + "'}",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (msg) {
                var m_MsgData = jQuery.parseJSON(msg.d);
                ...
                setTimeout('loadPowerData()', 60000);
            },
            error: function () {
                setTimeout('loadPowerData()', 60000);
            }
